using System.Net;
using System.Text;
#if ((NETSTANDARD2_0_OR_GREATER || NETCOREAPP3_1) && !NET6_0_OR_GREATER)
using System;
using System.IO;
using System.Linq;
...
public static class ProfilerRequestDelegates
{
	public static async Task MainPathInvokeAsync(HttpContext context)
	{
	...
	public static async Task DetailPathInvokeAsync(HttpContext context)
	{
		var buffer = ProfileViewExportProcessor.SessionBuffer.ToArray();
...
#else
		var traceId = (string?)context.Features.Get<IRoutingFeature>()!.RouteData.Values["traceId"];
#endif
	...
	public static async Task CssPathInvokeAsync(HttpContext context)
	{
		var resourceFileProvider = context.RequestServices.GetRequiredService<IResourceFileProvider>();

		context.Response.ContentType = "text/css";
		using var cssStream = resourceFileProvider.GetFileInfo("treeview_timeline.css").CreateReadStream();
		using var sr = new StreamReader(cssStream);

		await context.Response.WriteAsync(sr.ReadToEnd());
	}

	public static async Task IconPathInvokeAsync(HttpContext context)
	{
		var resourceFileProvider = context.RequestServices.GetRequiredService<IResourceFileProvider>();

		context.Response.ContentType = "image/png";

		using var iconsStream = resourceFileProvider.GetFileInfo("icons.png").CreateReadStream();
		using var br = new BinaryReader(iconsStream);

#if NETCOREAPP3_1_OR_GREATER
		await context.Response.Body.WriteAsync(
			br.ReadBytes((int)iconsStream.Length).AsMemory(0, (int)iconsStream.Length));
#else
